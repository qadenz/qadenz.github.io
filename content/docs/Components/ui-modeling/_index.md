...
Leaving the `PageFactory` behind is not a contrarian streak. Simon Stewart, the creator of WebDriver, has [told teams to stop using it](https://www.youtube.com/watch?v=gyfUpOysIF8&t=1519s) and find a better way. The `@FindBy` field is convenient to write and expensive to live with: it binds a name to a selector and nothing else, and it hands back an element that the next re-render can invalidate. Qadenz takes the advice at face value and builds its UI model on the `Locator` instead, a plain value that outlives any single render.

The late binding is the hinge between this section and [Commands]({{< relref "/docs/Components/Commands/_index.md" >}}). A `Locator` never holds a `WebElement`. When a command receives one, the [`WebFinder`]({{< relref "webfinder.md" >}}) resolves it against the live DOM at that instant, waits for it if the command calls for a wait, and discards it after. The model is a set of descriptions; the elements are momentary. This is why a Qadenz page object does not go stale between steps: there is nothing held to go stale.
